	...
	function play(){
      ...
      // 直接点到13
       if (pnum===13) {
        ...
        //   })
        //   }else{
        //     prev.animate({top:"+=20"})  //前一个和当前的合并成一个集合删除border
        //   }
        //      prev=null;//无论是不是13都把prev清空
        // }else{
          if (getpnum(prev)==getpnum(this)) {
                 prev.animate({top:"+=20"});
                 // prev=null;

          }else{
             if (getpnum(prev)+getpnum(this)===13) {
               prev.add(this).animate({top:-300})
               .queue(function(){
                $(this).detach().dequeue()
               })
            }else{
